...
; Initial n si max sunt amandoua 0
; Apoi daca sunt egale se va apela functia help cu urmatoarele elemente
; din g, h va ramane acelasi, iar n va creste. Daca nu sunt egale,
; se va apela cu restul din h, n va ramane 0 iar maximul va creste.
; Asa se vor forma 0 . 1 0 . 2 apoi 1 . 2 si tot asa
(define (pairs G H)
...
;(stream->list (stream-take (pairs naturals (stream-rest naturals)) 10)) 
; TODO
...
; odd va forma perechile impare, asemanator cu naturals doar ca se incepe de la 1 si se creste cu 2
(define odd
  (let nat ([seed 1])
...
;(stream->list (stream-take gh-pairs-stream 20))
; TODO
; Definiți fluxul de TPP corespunzător fluxului anterior de
; perechi (g, h).

; g va fi prima parte din pair, iar h restul
;;    a = gh
;;    b = 2ef = (h - g)(h + g) / 2
;;      = (h^2 - g^2) / 2
;;    c = e^2 + f^2 = (h - g)^2 / 4 + (h + g)^2 / 4
